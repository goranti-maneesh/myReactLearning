...
import TablistImageView from './TabListImageView'
...
class App extends Component {
  state = {
    Score: 0,
    randomImageUrlState: '',
    categoryState: tabsList[0].tabId,
    randomImageIdState: '',
    gameOver: false,
  }
  ...
  matchImageAfterClick = thumnailImageId => {
    const {randomImageIdState} = this.state
    if (thumnailImageId === randomImageIdState) {
      this.setState(prevState => ({
        Score: prevState.Score + 1,
      }))
      this.randomImageFunction()
    } else {
  ...
  onClicktabChange = clickedTabId => {
    this.setState({categoryState: clickedTabId})
  }

  render() {
    const {Score, randomImageUrlState, categoryState, gameOver} = this.state
    ...
    return (
      <>
        {gameOver ? (
          <GameOver
            scoreProp={Score}
            refreshDataFunctionProp={this.refreshDataFunction}
          />
        ) : (
          <div>
            <div className="css-App-header-div">
              <Header
                scoreProp={Score}
                gameOverProp={this.timeUpGameOver}
                gameOverValueProp={gameOver}
              />
            </div>
            <div className="css-App-non-header-div">
              <RandomImage imageUrlProp={randomImageUrlState} />
              <TabList
                tabListProps={tabsList}
                activeIdprop={categoryState}
                onClickFunction={this.onClicktabChange}
              />
              <TablistImageView
                imageListProps={imagesList}
                activeIdProp={categoryState}
                onClikFunctionProp={this.matchImageAfterClick}
              />
            </div>
          </div>
        )}
